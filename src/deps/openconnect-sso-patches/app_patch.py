"""
Patched app.py for openconnect-sso with external browser support.
"""
...
async def _run(args, cfg):
    ...
    # PATCH: Pass external_browser flag
    external_browser = getattr(args, 'external_browser', False)
...
def authenticate_to(host, proxy, credentials, display_mode, version, external_browser=False):
    logger.info("Authenticating to VPN endpoint", name=host.name, address=host.address)
    return Authenticator(host, proxy, credentials, version).authenticate(display_mode, external_browser=external_browser)
